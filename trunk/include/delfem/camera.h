...
public:
  ...
  CVector3D GetCenterPosition() const{
  ...
  /////
  virtual void MouseRotation(const double& mov_begin_x, const double& mov_begin_y,
                     const double& mov_end_x, const double& mov_end_y ) = 0;
  virtual void RotMatrix33(double m[]) const = 0;
  virtual Com::CMatrix3 GetRotMatrix3() const{
...
protected:
	...
	double win_center_x, win_center_y; // where the center of obj locate in screen (-1..+1,-1..+1)
	double inv_scale;
...
public:
  ...
  Com::CVector3D ProjectionOnPlane(const double pos_x,   const double pos_y,
...
class CCamera_2DH : public CCamera
{
public:
  CCamera_2DH(){
		theta = 0;
    phi = 0;//-60.0*3.1415926/180.0;      
    iaxis = 2;
  }  
  void RotMatrix33(double m[]) const {
    const double ct = cos(theta);
    ...
    if( iaxis == 1 ){
      m[0] =    ct; m[1] =     0; m[2] =  st;
      m[3] = +sp*st; m[4] =    cp; m[5] = -sp*ct;
      m[6] = -cp*st; m[7] =   +sp; m[8] =  cp*ct;          
    }    
    if( iaxis == 2 ){
      m[0] =    ct; m[1] =   -st; m[2] = 0.0;
  ...
  void SetRotAxis(unsigned int iax0){
    iaxis = iax0;
  }
	void MouseRotation(const double& mov_begin_x, const double& mov_begin_y,
                     const double& mov_end_x, const double& mov_end_y )
  ...
  {
    const double hvh = this->GetHalfViewHeight()/this->GetScale();
    const double asp = this->GetWindowAspect(); 
    src.x = scr_x*hvh*asp; 
    src.y = scr_y*hvh;
    src.z = 0;
    double trans1[3]; this->GetCenterPosition(trans1[0],trans1[1],trans1[2]);
    src.x -= trans1[0];
    src.y -= trans1[1];
    src.z -= trans1[2];
    double m[9]; this->RotMatrix33(m);
    Com::CVector3D src0 = src;
    src.x = m[0]*src0.x + m[3]*src0.y + m[6]*src0.z;
    src.y = m[1]*src0.x + m[4]*src0.y + m[7]*src0.z;
    src.z = m[2]*src0.x + m[5]*src0.y + m[8]*src0.z;    
    src += obj_center;
    dir.x = m[6];
    dir.y = m[7];
    dir.z = m[8];    
...
public:
  unsigned int iaxis; // 0:x-axis, 2:y-axsis, 3:z-axis
  double theta;  
  double phi;
};

class CCamera_3D : public CCamera
{
public:
	CCamera_3D(){
    rot_quat.SetUnit();    
  ...
  Com::CVector3D ProjectionOnPlane(
    ...
    const double hvh = this->GetHalfViewHeight()/this->GetScale();
    const double asp = this->GetWindowAspect(); 
    Com::CVector3D p( pos_x*hvh*asp, pos_y*hvh, 0 );
    double trans1[3]; this->GetCenterPosition(trans1[0],trans1[1],trans1[2]);
    p.x -= trans1[0];
    p.y -= trans1[1];
    p.z -= trans1[2];
    p = UnRotate(rot_quat,p);
    p += obj_center;
    Com::CVector3D d = UnRotate(rot_quat,Com::CVector3D(0,0,1));
    
    Com::CVector3D n(norm_x,norm_y,norm_z);
    double t = Dot(Com::CVector3D(plane_x,plane_y,plane_z)-p,n)/Dot(d,n);
    return p+t*d;        
	}
  // -1 < srcx < 1
  // -1 < srcy < 1
  void GetScrPos_from3DPos(double& scrx, double& scry, const Com::CVector3D& v) const{
    double trans0[3]; this->GetObjectCenter(trans0[0],trans0[1],trans0[2]);
    double rot[9];    this->RotMatrix33(rot);
    double trans1[3]; this->GetCenterPosition(trans1[0],trans1[1],trans1[2]);
    const double hvh = this->GetHalfViewHeight()/this->GetScale();
    const double asp = this->GetWindowAspect();    
    const double c1[3] = { v.x-trans0[0], v.y-trans0[1], v.z-trans0[2] };
    double c2[3] = {
      rot[0]*c1[0] + rot[1]*c1[1] + rot[2]*c1[2],
      rot[3]*c1[0] + rot[4]*c1[1] + rot[5]*c1[2],
      rot[6]*c1[0] + rot[7]*c1[1] + rot[8]*c1[2] };
    c2[0] += trans1[0];
    c2[1] += trans1[1];    
    c2[2] += trans1[2];        
    scrx = c2[0]/(hvh*asp);
...
private:
	CQuaternion rot_quat; // 3d rotation
};

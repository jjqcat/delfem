...
unsigned int CBRep2D::CItrLoop::GetType() const // 0:浮遊点 1:浮遊辺 2:面積がある
{
...
//  std::cout << "CBRep2D::ConnectVertex" << std::endl;
	...
	if( id_ul1 != id_ul2 ){	// 親ループと子ループ、または子ループ同士を結ぶ。MEKL系
		if( id_l != 0 ){
			////////////////
			if( is_float1 && !is_float2 ){	// 浮遊点とループを結ぶ
				assert( !is_float2 );
//				std::cout << "Add Edge In Loop One Floating Vertex(1)" << std::endl;
			...
			////////////////
			else if( is_float2 && !is_float1 ){	// 浮遊点とルーﾌﾟを結ぶ
				assert( !is_float1 );
//				std::cout << "Add Edge In Loop One Floating Vertex(2)" << std::endl;
			...
			////////////////
			else if( is_float1 && is_float2 ){	// 浮遊点同士を結ぶ
...
//				std::cout << "Add Edge with Child Loop " << std::endl;
		...
		////////////////
		else if( id_l == 0 ){	// くっついていないループ同士を辺で結ぶ
...
//				std::cout << "Add Edge In Loop One Outer Floating Vertex(2)" << std::endl;
	...
	else if( id_ul1 == id_ul2 ){	// ループを２つに分ける。MEL系
...
//			assert( he1.id_he_b == id_he2 );
	...
	////////////////////////////////
	if( id_l1 != id_l2 && id_l1 != 0 && id_l2 != 0 ){	// ２つのループを隔てていた辺を消す
//		std::cout << "Remove Splited Edge" << std::endl;
		assert( id_ul1 != id_ul2 );
		assert( !is_edge_vertex1 );
		assert( !is_edge_vertex2 );
		// 子ループがあればそちらが残るように消すと、ループの入れ替えが楽でいい。
		// 両方子ループってことは多分ないので、つまり親ループを積極的に消せばよい
		{
			if( !m_BRep.KEL(id_he1) ) assert(0);
			unsigned int id_ul1p;	// id_ul1の新しい位相親ループ
			unsigned int id_l1p;	// id_ul1の新しい幾何親ループ
			{
				const CUseLoop& ul1 = m_BRep.GetUseLoop(id_ul1);
				id_ul1p = ul1.id_ul_p;
				if( id_ul1p == 0 ) id_ul1p = id_ul1;

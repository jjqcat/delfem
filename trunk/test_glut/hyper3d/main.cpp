...
void SetNewProblem()
{
	const unsigned int nprob = 1;	// 問題数
...
//        Msh::CMesh_Primitive_ThickCylinder mesh_3d(2,2.5,20, 1, 12, 6);
		...
		unsigned int id_field_bc1 = world.GetPartialField(id_field_disp,conv.GetIdEA_fromMsh(2));
		{
			...
			bc1_field.SetValue("1*sin(2*t)", 0,Fem::Field::VALUE, world,true);	// bc1_fieldのy座標に単振動を追加
			bc1_field.SetValue("1*sin(t)"  , 1,Fem::Field::VALUE, world,true);	// bc1_fieldのy座標に単振動を追加
//			bc1_field.SetValue("-1*sin(t)" , 2,Fem::Field::VALUE, world,true);	// bc1_fieldのy座標に単振動を追加
		}

...
void StepTime3()
{
  ...
  ////////////////
  ls.InitializeMarge();
  ls.UpdateValueOfRigidSystem_NewmarkBetaAPrime(aRB,apFix,   dt,newmark_gamma,newmark_beta,     true);
  double norm_res0;
	for(unsigned int itr=0;itr<10;itr++){
    ls.InitializeMarge();
    for(unsigned int irb=0;irb<aRB.size();irb++){
      aRB[irb].AddLinearSystem_NewmarkBetaAPrime(ls,irb,      dt,newmark_gamma,newmark_beta,      gravity,itr==0 );
    }
    for(unsigned int ifix=0;ifix<apFix.size();ifix++){
      apFix[ifix]->AddLinearSystem_NewmarkBetaAPrime(ls,ifix,         dt,newmark_gamma,newmark_beta,       aRB, itr==0);
    }
    ////////////////////////////////
...
//    std::cout << "itr : " << itr << "     Residual : " << res << std::endl;    
    ...
    ////////////////////////////////
    ls.UpdateValueOfRigidSystem_NewmarkBetaAPrime(aRB,apFix,   dt,newmark_gamma,newmark_beta,     false);
...
void DrawRigidBody(const Rigid::CRigidBody3D& r){
  ...
  if( imode == 0 ){
    ...
    ::glPushMatrix();
    {
      const Com::CVector3D& v = r.GetIniPosCG() + r.GetDispCG();
      ::glTranslated(v.x,v.y,v.z);    
    }
    {
      Com::CMatrix3 m = r.GetRotMatrix();      
      double rot0[16];
...
{	
    ...
    const Com::CVector3D& vec_j = rb.GetCurrentPositionFromInitial(cfh->ini_pos_fix);
    const Com::CVector3D& vec_cg = rb.GetIniPosCG() + rb.GetDispCG();
   ...
   */
    ...
    const Com::CVector3D& vec_j = rb.GetCurrentPositionFromInitial(ini_pos_fix);
    const Com::CVector3D& vec_cg = rb.GetIniPosCG() + rb.GetDispCG();
    ...
    const Com::CVector3D& vec_j = rb0.GetCurrentPositionFromInitial(cjh->ini_pos_joint);
    const Com::CVector3D& vec_cg0 = rb0.GetIniPosCG() + rb0.GetDispCG();
    const Com::CVector3D& vec_cg1 = rb1.GetIniPosCG() + rb1.GetDispCG();
    ...
    //		std::cout << "CJoint_Spherical" << std::endl;			
		...
		const Com::CVector3D& vec_j = rb0.GetCurrentPositionFromInitial(cjs->ini_pos_joint);
...
void SetNewProblem(){
  ...
  if( iprob == 0 ){
    aRB.resize(1);
    aRB[0].SetIniPosCG( Com::CVector3D(1.0,0,0) );
    ...
    for(unsigned int irb=0;irb<nRB;irb++){
      aRB[irb].SetIniPosCG( Com::CVector3D(div_len*(irb+1),0,0) );
      if( irb == 0 ){
    ...
    {
      aRB[0].SetIniPosCG( Com::CVector3D(1.0,0,0) );
      aRB[1].SetIniPosCG( Com::CVector3D(1.0,1,0) );
      aRB[2].SetIniPosCG( Com::CVector3D(2.0,1,0) );
    }
    {
  ...
  /*
    ...
    aRB.resize(3);
    aRB[0].SetIniPosCG( Com::CVector3D(1.0,0,0) );
    aRB[1].SetIniPosCG( Com::CVector3D(2.0,0,0) );
    aRB[2].SetIniPosCG( Com::CVector3D(3.0,0,0) );
    {
      Rigid::CFix_HingeRange* pFix = new Rigid::CFix_HingeRange(0);
      pFix->SetIniPosFix(0,0,0);
      pFix->SetAxis(0,1,0);
    ...
    aRB[1].SetIniPosCG( Com::CVector3D(2.0,0,0) );
    aRB[2].SetIniPosCG( Com::CVector3D(3.0,0,0) );
    aRB[3].SetIniPosCG( Com::CVector3D(4.0,0,0) );
    {
      Rigid::CFix_HingeRange* pFix = new Rigid::CFix_HingeRange(0);
      pFix->SetRange(-30,35);
      //            Rigid::CFix_Hinge* pFix = new Rigid::CFix_Hinge(0);
  ...
  if( iprob == 8 ){
    aRB.resize(1);
    aRB[0].SetIniPosCG( Com::CVector3D(0.3,0.0,1.0) );
    aRB[0].SetOmega( Com::CVector3D(0,0,1) );
    double mineatia[3] = { 1, 1, 10 };
    aRB[0].SetMass(0.001, mineatia );
    Rigid::CFix_Spherical* pFix = new Rigid::CFix_Spherical(0);
    pFix->SetIniPosFix(0,0,0);
    apFix.push_back( pFix );
  }

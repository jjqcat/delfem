...
void GLWidget_Solid2d::SetNewProblem()
{
    ...
    if( iprob == 0 )
    {
        ...
        drawer_ary.InitTrans(camera);	// View?A?W?I?ÅE?I?YÅfe
    }
    else if( iprob == 1 )	// ???Å·?s?n?d?UÅeÅ˜?É ?A?C???AÅgI?aÅee
    {
        solid.SetSaveStiffMat(true);
    }
    else if( iprob == 2 )	// ???Å·?s?n?d?UÅeÅ˜?É ?A?CÅgRÅgI?aÅee?dÅÒd?-
    {
        solid.SetSaveStiffMat(true);
        solid.SetStationary(false);
    }
    else if( iprob == 3 )	// ?-?É ?A?-?ÅE?e
    {
        solid.SetYoungPoisson(50,0.3,true);
    }
    else if( iprob == 4 )	// ?a???-?É ?A?-?ÅE?e
    {
        solid.SetYoungPoisson(100,0.3,true);
    }
    else if( iprob == 5 )	// ?oÅÒ??wÅgIÅhn?u?`?d?l?|???C???AÅgIÅÒd?I
    {
        solid.SetStationary(true);
        solid.SetGeometricalNonlinear(true);
    }
    else if( iprob == 6 )	// ?oÅÒ??wÅgIÅhn?u?`?d?l?|???CÅgRÅgIÅÒd?I
    {
        solid.SetYoungPoisson(10,0.0,true);
        solid.SetStationary(false);
        solid.SetGeometricalNonlinear(true);
    }
    else if( iprob == 7 )	// ?u?`?~?[?[?XÅÒ??I?I?ÅÒ?u?EÅfu?A?I?\?|
    {
        ...
        drawer_ary.PushBack( new Fem::Field::View::CDrawerEdge(id_field_disp,false,world) );
        drawer_ary.PushBack( new Fem::Field::View::CDrawerEdge(id_field_disp,true ,world) );
        // View?A?W?I?ÅE?I?YÅfe
        drawer_ary.InitTrans(camera);
    }
    else if( iprob == 8 )	// ?u?`?~?[?[?XÅÒ??I?I?ÅÒ?u?EÅfu?A?I?\?|
    {
        // ?`ÅÒa?I?u?W?F?N?g?IÅgo?^
        drawer_ary.Clear();
        id_field_disp = solid.GetIdField_Disp();
    ...
    else if( iprob == 10 )	// ÅhMÅÒ??I?d?l?Å˜?ÅE?e?Å}?A?d?a?s?e
    {
        drawer_ary.Clear();
    ...
    else if( iprob == 11 )	// ÅhMÅÒ??I?d?l?Å˜?ÅE?e?Å}?A?d?a?s?e
    {
...
//		drawer_ary.PushBack( new View::CDrawerFace(id_field_disp,false,world) );
    ...
    else if( iprob == 27 )
    {
        ...
        unsigned int id_base2 = 0;
        {
            ...
            {
                std::vector<unsigned int> aIdMsh_Inc;
                aIdMsh_Inc.push_back( mesh_2d.GetElemID_FromCadID(id_l,Cad::LOOP) );	// ?J?b?g?3?e?e???[?v?dÅgo?^
                std::vector<unsigned int> aIdMshCut;
                aIdMshCut.push_back( mesh_2d.GetElemID_FromCadID(id_e1,Cad::EDGE) );	// ?J?b?g?ÅE?e?O?dÅgo?^
                aIdMshCut.push_back( mesh_2d.GetElemID_FromCadID(id_e2,Cad::EDGE) );	// ?J?b?g?ÅE?e?O?dÅgo?^
                mesh_2d.GetClipedMesh(aLnods,mapVal2Co, aIdMsh_Inc,aIdMshCut);
            }
            std::vector<unsigned int> aIdEA_Inc;
            aIdEA_Inc.push_back( conv.GetIdEA_fromCad(id_l,Cad::LOOP) );
        ...
        {
            CField& bc1_field = world.GetField(id_field_bc0);
            bc1_field.SetValue("0.2*sin(t*PI*2*0.1)", 0,Fem::Field::VALUE, world,true);	// bc1_field?Iy?A?W?EÅfP?UÅgR?dÅfCÅÒA
        }
        id_field_disp = solid.GetIdField_Disp();

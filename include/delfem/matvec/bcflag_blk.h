...
namespace MatVec{
  ...
  public:
    ...
    void SetAllFlag(int val=0){
      ...
      for(unsigned int idof=0;idof<ndof;idof++){ m_Flag[idof] = val; }
    }
    ...
    inline bool SetBC(unsigned int iBlk, unsigned int iDofBlk, int val=1){
      assert( iBlk < m_nBlk );
      if( this->m_lenBlk == -1 ){
        assert( m_DofPtr != 0 );
        assert( iDofBlk < m_DofPtr[iBlk+1]-m_DofPtr[iBlk] );
        m_Flag[ m_DofPtr[iBlk]+iDofBlk ]=val;
        return true;
      }
      assert( (int)iDofBlk < m_lenBlk );
      m_Flag[iBlk*m_lenBlk+iDofBlk] = val;
      return true;
    }
  private:

...
const CEdge2D& CCadObj2D::GetEdge(unsigned int id_e) const
{
  ...
  e.SetVtxCoords( this->GetVertexCoord(id_vs), this->GetVertexCoord(id_ve) );
	return e;
}
...
CURVE_TYPE CCadObj2D::GetEdgeCurveType(const unsigned int& id_e) const
{
	assert( m_EdgeSet.IsObjID(id_e) );
	const CEdge2D& e = m_EdgeSet.GetObj(id_e);
	return e.GetCurveType();
}
        ...
        unsigned int id_e, std::vector<Com::CVector2D>& aCo, double elen) const
{
  ...
  if( elen > 0 ){
    ...
    return e.GetCurveAsPolyline(aCo, ndiv);
  }
  return e.GetCurveAsPolyline(aCo,-1);  
}
...
double CCadObj2D::GetArea_ItrLoop(CBRepSurface::CItrLoop& itrl) const
{
    ...
    assert( this->IsElemID(Cad::EDGE,id_e) );
		const CEdge2D& e = this->GetEdge(id_e);
		assert( e.GetIdVtx( is_same_dir) == itrl.GetIdVertex() );
		assert( e.GetIdVtx(!is_same_dir) == itrl.GetIdVertex_Ahead() );    
		// add area here
		const double earea = TriArea(e.GetVtxCoord(true), e.GetVtxCoord(false), CVector2D(0.0,0.0)) + e.AreaEdge();
		...
		for(CBRepSurface::CItrLoop itrl=m_BRep.GetItrLoop(id_l);!itrl.IsEndChild();itrl.ShiftChildLoop()){
			for(itrl.Begin();!itrl.IsEnd();itrl++){	// ループの中の点をめぐる
				...
				Com::CVector2D dir;
				{
					...
					assert( e0.GetIdVtx( is_same_dir0) == id_vm );
					assert( e0.GetIdVtx(!is_same_dir0) == id_vf );
				}
				for(CBRepSurface::CItrVertex itrv=m_BRep.GetItrVertex(id_vm);!itrv.IsEnd();itrv++){	// 点周りの辺をめぐる
					...
					assert( e0.GetIdVtx(is_same_dir0) == id_vm );
					if( e0.GetIdVtx(!is_same_dir0) == id_vf ){ continue; }
					...
					assert( e1.GetIdVtx(is_same_dir1) == id_vm );
					if( e1.GetIdVtx(!is_same_dir1) == id_vf ){ continue; }
...
CBRepSurface::CItrVertex CCadObj2D::FindCorner_HalfLine(unsigned int id_v, const CVector2D& dir1) const
{
	...
	for(;!itrv.IsEnd();itrv++){
		...
		const Cad::CEdge2D& e0 = this->GetEdge(id_e0);
		assert( e0.GetIdVtx(is_same_dir0) == id_v );
		const CVector2D& tan0 = e0.GetTangentEdge(is_same_dir0);
		////////////////
		unsigned int id_e1;   bool is_same_dir1;
		itrv.GetIdEdge_Ahead(id_e1,is_same_dir1);
		...
		const Cad::CEdge2D& e1 = this->GetEdge(id_e1);
		assert( e1.GetIdVtx(is_same_dir1) == id_v );
		const CVector2D& tan1 = e1.GetTangentEdge(is_same_dir1);
		////////////////
...
{
	const unsigned int id_v1 = edge.GetIdVtx(true);
	const unsigned int id_v2 = edge.GetIdVtx(false);
  ...
  ////  
	...
	if( edge.GetCurveType() == CURVE_LINE ){	// check if there is line edge with end points (v1,v2)
		...
		for(unsigned int iid_e=0;iid_e<id_ary_e.size();iid_e++){
			...
			const CEdge2D& e = this->GetEdge(id_e);
			if( e.GetCurveType() != CURVE_LINE ) continue;
			const unsigned int id_v_s = e.GetIdVtx(true);      
			const unsigned int id_v_e = e.GetIdVtx(false);
			if( (id_v_s-id_v1)*(id_v_s-id_v2) != 0 ) continue;
			if( (id_v_e-id_v1)*(id_v_e-id_v2) != 0 ) continue;
			return res;
		}
	}
	edge.SetVtxCoords(m_VertexSet.GetObj(id_v1).point, m_VertexSet.GetObj(id_v2).point);
    ...
    bool is_left_ladd = false;    
      ...
      // array of edge that begin id_v2 and end id_v1 that is part of loop generated if v1 and v2 are connected
			...
			double area = TriArea(edge.GetVtxCoord(true), edge.GetVtxCoord(false), CVector2D(0,0)) + edge.AreaEdge();
			for(unsigned int ie=0;ie<aIdEDir.size();ie++){
				unsigned int id_e = aIdEDir[ie].first;
				assert( this->IsElemID(Cad::EDGE,id_e) );
				const CEdge2D& e = this->GetEdge(id_e);
				const double earea = e.AreaEdge() + TriArea(e.GetVtxCoord(true), e.GetVtxCoord(false), CVector2D(0,0));
...
bool CCadObj2D::RemoveElement(Cad::CAD_ELEM_TYPE itype, unsigned int id)
{
	...
	{ 
			...
			CEdge2D e_tmp = this->GetEdge(id_e1);
      {
        ...
        if( btmp1 ){ 
          e_tmp.SetIdVtx(id_v2,e_tmp.GetIdVtx(false));
          e_tmp.SetVtxCoords(this->GetVertexCoord(id_v2), e_tmp.GetVtxCoord(false));
//          e_tmp.id_v_s = id_v2; 
//          e_tmp.po_s = this->GetVertexCoord(id_v2); 
        }
        else{        
          e_tmp.SetIdVtx(e_tmp.GetIdVtx(true),id_v2);
          e_tmp.SetVtxCoords(e_tmp.GetVtxCoord(true),this->GetVertexCoord(id_v2));
//          e_tmp.id_v_e = id_v2; 
//          e_tmp.po_e = this->GetVertexCoord(id_v2); 
        }
      }
      ////////////////
      {   // 辺がとe_tmpが交差していないかを全ての辺について調べる
        const unsigned int ipo0 = e_tmp.GetIdVtx(true);
        const unsigned int ipo1 = e_tmp.GetIdVtx(false);
...
CCadObj2D::CResAddPolygon Cad::CCadObj2D::AddLoop
(const std::vector< std::pair<Cad::CURVE_TYPE, std::vector<double> > >& aVal, 
 unsigned int id_l,
 double scale)
{
  CResAddPolygon res;
	const unsigned int npoint = aVal.size();
	res.aIdV.reserve(npoint);
  std::vector<CVector2D> aVec;
	for(unsigned int ipoint=0;ipoint<npoint-1;ipoint++){
    const std::vector<double>& ad = aVal[ipoint].second;
    Com::CVector2D p( ad[ad.size()-2]*scale,ad[ad.size()-1]*scale );
    aVec.push_back(p);
		unsigned int id_v0 = this->AddVertex(Cad::LOOP, id_l, p).id_v_add;
  ...
  for(unsigned int iedge=0;iedge<npoint-1;iedge++){
    unsigned int ips = iedge;    
    unsigned int ipe = (iedge!=npoint-2) ? iedge+1 : 0;    
    Cad::CEdge2D e(res.aIdV[ips], res.aIdV[ipe] );
//    std::cout << iedge << " " << npoint << "    " << res.aIdV[ips] << " " << res.aIdV[ipe] << "  " << std::endl;
    if( iedge != npoint-1 ){
      CURVE_TYPE type = aVal[iedge+1].first;
      if( type == CURVE_BEZIER ){
        const std::vector<double>& ad = aVal[iedge+1].second;
        const CVector2D& vs = aVec[ips];
        const CVector2D& ve = aVec[ipe];
        const CVector2D vsc = CVector2D(ad[0]*scale,ad[1]*scale);
        const CVector2D vec = CVector2D(ad[2]*scale,ad[3]*scale);
        CVector2D vh = CVector2D(ve-vs);
        {
          double len = vh.Length();
          vh *= 1.0/(len*len);
        }
        CVector2D vv = CVector2D(-vh.y,vh.x);
        double t0 = Dot(vsc-vs,vh);
        double t1 = Dot(vsc-vs,vv);
        double t2 = Dot(vec-vs,vh);
        double t3 = Dot(vec-vs,vv);
        std::cout << t0 << " " << t1 << " " << t2 << " " << t3 << std::endl;
        e.SetCurve_Bezier(t0,t1,t2,t3);
      }
    }
    unsigned int id_e0 = this->ConnectVertex(e).id_e_add;
    std::cout << "edge add " << id_e0 << std::endl;
  ...
  std::vector<Com::CVector2D> aPoint = aPoint_input;
	{	// 交線判断
		...
		for(unsigned int i=0;i<n-1;i++){
			CEdge2D e(i,i+1);
      e.SetVtxCoords(aPoint_input[i  ], aPoint_input[i+1]);
			aEdge.push_back( e );
		}
		{
			CEdge2D e(n-1,0);
      e.SetVtxCoords(aPoint_input[n-1], aPoint_input[0]);
			aEdge.push_back( e );
		}
...
//  std::cout << "CadObj2D::AddVertex" << std::endl;
  CResAddVertex res;
	...
	else if( itype == Cad::EDGE )
	{
		...
		if(   SquareLength(vec_add-edge_old.GetVtxCoord(false)) < 1.0e-20 
       || SquareLength(vec_add-edge_old.GetVtxCoord(true)) < 1.0e-20 ){ return res; }
		////////////////////////////////
		// Leave Input Check Section
		...
		{	// add edge
			const int tmp_id = m_EdgeSet.AddObj( std::make_pair(id_e_add,CEdge2D(id_v_add,edge_old.GetIdVtx(false))) );
		...
		}
		{
			...
			assert( edge.GetIdVtx(false) == id_v_add );
			edge = edge_old;
		}
		assert( this->AssertValid() == 0 );
    res.id_v_add = id_v_add;
...
//		std::cout << "SetCurve_Arc Failure(this Edge is not exist)" << std::endl;
  ...
  e.SetCurve_Arc(is_left_side,rdist);
  // ここまで
  ////////////////////////////////
	const std::vector<unsigned int>& aID_Loop = m_LoopSet.GetAry_ObjID();
	for(unsigned int iid_l=0;iid_l<aID_Loop.size();iid_l++){
...
//    std::cout << "SetCurve_Arc Failure(this Edge is not exist)" << std::endl;
  ...
  ////////////////////////////////
  // ここからを現在のCurveTypeによって決める,次の設定は直線の場合
  const CVector2D& pos = e_old.GetVtxCoord(true);
  const CVector2D& poe = e_old.GetVtxCoord(false);
  std::vector<Com::CVector2D> aCo;
  e_old.GetCurveAsPolyline(aCo,20);
...
//		std::cout << "SetCurve_Arc Failure(this Edge is not exist)" << std::endl;
    ...
    std::vector<double> aRelCo;
		aRelCo.reserve(n*2);
		const Com::CVector2D& pos = e.GetVtxCoord(true);
		const Com::CVector2D& poe = e.GetVtxCoord(false);
		const double sqlen = Com::SquareLength(poe-pos);
		const Com::CVector2D& eh = (poe-pos)*(1/sqlen);
		const Com::CVector2D ev(-eh.y,eh.x);
...
{  
  //    std::cout << "CCadObj2D::SetCurve_Line" << id_e << std::endl;
	if( !m_EdgeSet.IsObjID(id_e) ){ return false; }
	assert( m_EdgeSet.IsObjID(id_e) );
  CEdge2D& e = m_EdgeSet.GetObj(id_e);
  CEdge2D e_old = e;
	////////////////
	e.SetCurve_Bezier(cx0,cy0,cx1,cy1);
	////////////////
	const std::vector<unsigned int>& aID_Loop = m_LoopSet.GetAry_ObjID();
	for(unsigned int iid_l=0;iid_l<aID_Loop.size();iid_l++){
		unsigned int id_l = aID_Loop[iid_l];
...
bool CCadObj2D::CheckIntersection_Loop(unsigned int id_l) const
{
  ...
  if( m_BRep.IsElemID(Cad::LOOP,id_l) ){
    ...
    const CEdge2D& edge = this->GetEdge( aIdEdge[ie] );
		...
		const unsigned int ipo0 = edge.GetIdVtx(true);
		const unsigned int ipo1 = edge.GetIdVtx(false);
    ...
    CBRepSurface::CItrLoop pItr = m_BRep.GetItrLoop(id_l);
      ...
      for(;!pItr.IsEnd();pItr++){
        ...
        unsigned int id_ve = this->GetIdVertex_Edge(id_e, false); assert( this->IsElemID(Cad::VERTEX,id_ve) );
				...
				if( this->GetEdgeCurveType(id_e) == CURVE_LINE ){
					fprintf(fp,"  0\nLINE\n  8\n%d\n  6\nCONTINUOUS\n  62\n7\n",id_l);
					fprintf(fp,"  10\n%lf\n",ps.x*scale);
					fprintf(fp,"  20\n%lf\n",ps.y*scale);
					fprintf(fp,"  11\n%lf\n",pe.x*scale);
				...
				else if( this->GetEdgeCurveType(id_e) == CURVE_ARC ){ // Arc
					const CEdge2D& edge = this->GetEdge(id_e);
            ...
            bool is_left_side; double dist; edge.GetCurve_Arc(is_left_side,dist);
						if( is_left_side ){ d1 = de; d2 = ds; }
						else{               d1 = ds; d2 = de; }
					}
					fprintf(fp,"  0\nARC\n  8\n%d\n  6\nCONTINUOUS\n  62\n7\n  100\nAcDbCircle\n",id_l);
					fprintf(fp,"  10\n%lf\n",pc.x*scale);	// x coord
        ...
        else if( this->GetEdgeCurveType(id_e) == CURVE_POLYLINE ){ // polyline
					const CEdge2D& edge = this->GetEdge(id_e);
					fprintf(fp,"  0\nPOLYLINE\n  8\n%d\n  6\nCONTINUOUS\n",id_l);
					fprintf(fp,"  10\n0.0\n");
					fprintf(fp,"  20\n0.0\n");
        ...
        else if( this->GetEdgeCurveType(id_e) == CURVE_BEZIER ){ // bezier
					const CEdge2D& edge = this->GetEdge(id_e);
					fprintf(fp,"  0\nPOLYLINE\n  8\n%d\n  6\nCONTINUOUS\n",id_l);
          fprintf(fp,"100\n");
          fprintf(fp,"AcDbEntity\n");          
          fprintf(fp,"100\n");
          fprintf(fp,"AcDb2dPolyline\n");                    
					fprintf(fp,"  10\n0.0\n");
					fprintf(fp,"  20\n0.0\n");
					fprintf(fp,"  30\n0.0\n");
					fprintf(fp,"  66\n1\n");          
...
//			assert( nv>0 ); assert( ne>0 ); assert( nl>0 );
    ...
    for(int ie=0;ie<ne;ie++){
      ...
      CURVE_TYPE type;
			{
        int itype; arch.Get("%d",&itype);
        if(      itype == 0 ){ type=CURVE_LINE; }
        else if( itype == 1 ){ type=CURVE_ARC; }
        else if( itype == 2 ){ type=CURVE_POLYLINE; }
        else if( itype == 3 ){ type=CURVE_BEZIER; }
      }
			int i_is_left_side;
			double dist;
      ...
      if(      type == CURVE_LINE ){ e.SetCurve_Line(); }
      else if( type == CURVE_ARC ){ e.SetCurve_Arc(is_left_side,dist); }
      else if( type == CURVE_POLYLINE ){ e.SetCurve_Polyline(aRelCo); }
      else if( type == CURVE_BEZIER ){ // TODO:
      }
    ...
    { // print Edge2D
			...
			for(unsigned int iid=0;iid<id_ary.size();iid++){
				...
				arch.Out("%d\n",id_e);
				arch.Out("%d %d\n",e.GetIdVtx(true),e.GetIdVtx(false));
        {
          CURVE_TYPE type = e.GetCurveType();
          int itype;          
          if(      type == CURVE_LINE     ){ itype=0; }
          else if( type == CURVE_ARC      ){ itype=1; }
          else if( type == CURVE_POLYLINE ){ itype=2; }
          else if( type == CURVE_BEZIER   ){ itype=3; }
          arch.Out("%d\n",itype);
        }
        {
          bool is_left_side; double dist; e.GetCurve_Arc(is_left_side,dist);

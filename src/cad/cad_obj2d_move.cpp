...
{  
	if( !this->IsElemID(Cad::EDGE,id_e) ) return false;
	if( this->GetEdgeCurveType(id_e) != CURVE_ARC ) return true;
  
	bool is_left_side_old; double dist_old;
  {
    const Cad::CEdge2D& e = this->GetEdge(id_e);
    e.GetCurve_Arc(is_left_side_old,dist_old);
  }
  
  CEdge2D& edge = this->GetEdgeRef(id_e);
  assert( edge.GetCurveType() == CURVE_ARC );
  {
    const Com::CVector2D& ps = edge.GetVtxCoord(true);
    const Com::CVector2D& pe = edge.GetVtxCoord(false);    
    const double base_len = sqrt( SquareLength(ps,pe) );
    if( fabs(TriHeight(vec,ps,pe)) > base_len*0.02 ){
      CVector2D pc;
      Com::CenterCircumcircle(ps,pe,vec,pc);
      double dist = TriHeight(pc,ps,pe);
      edge.SetCurve_Arc(TriArea(ps,pe,vec) > 0, dist);
    }
    else{ return true; }  
  }
...
FAILURE:
  edge.SetCurve_Arc(is_left_side_old,dist_old);
	return true;  
}

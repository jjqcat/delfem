 ...
 std::vector<int>& aflg_ismov)
{
  ...
  aIndLoop.push_back(0);
    ...
    for(itrl.Begin();!itrl.IsEnd();itrl++){            
      ...
      if(      cad_2d.GetEdgeCurveType(id_e) == Cad::CURVE_LINE     ){ continue; } // line
      else if( cad_2d.GetEdgeCurveType(id_e) == Cad::CURVE_ARC      ){ continue; assert(0); } // arc
      else if( cad_2d.GetEdgeCurveType(id_e) == Cad::CURVE_POLYLINE ){ // polyline
...
{  
  ...
  if( itype_elem == Cad::VERTEX ){ 
    const unsigned int id_v = id_elem;
    aLamXY_Vtx[id_v*4+0] = 1;
    aLamXY_Vtx[id_v*4+3] = 1;    
  }
  if( itype_elem == Cad::EDGE ){
  ...
  if( itype_elem == Cad::LOOP ){
    unsigned int id_l = id_elem;
    for(Cad::CBRepSurface::CItrLoop itrl=cad_2d.GetItrLoop(id_l);!itrl.IsEnd();itrl.ShiftChildLoop()){
      for(itrl.Begin();!itrl.IsEnd();itrl++){        
  ...
  return this->Precomp_FitMeshToCad(cad_2d,aLamXY_Vtx);    
}
